...
class NodeTester {
  ...
  static Future<List<ProxyNode>> testAndSort(List<ProxyNode> nodes) async {
    if (nodes.isEmpty) {
      return nodes;
    }
    ...
    if (toTest.isNotEmpty) {
      await _runWithConcurrency(toTest, _maxConcurrency, (node) async {
        final result = await _testNode(node);
  ...
  static Future<_CacheEntry> _testNode(ProxyNode node) async {
    final now = DateTime.now();
    final hostPort = _extractHostPort(node.raw);
    final probeHost = _extractProbeHost(node.raw) ?? hostPort?.host;
    final probePort = hostPort?.port ?? 443;
    log('NodeTester: raw="${node.raw}" host="${probeHost ?? ''}" port="$probePort"');
    if (probeHost == null) {
      return _CacheEntry(
        latencyMs: null,
        available: false,
    ...
    final result = await _probeHttp(probeHost, probePort);
    stopwatch.stop();
    ...
    log('NodeTester: name="${node.displayName}" host="$probeHost" status=${result.statusCode} latencyMs=$latency');
  ...
  static Future<_ProbeResult> _probeHttp(String host, int port) async {
    ...
    for (final path in paths) {
      ...
      final result = await _performRequest(uri);
      lastResult = result;
      if (result.success) {
        return result;
      }
  ...
  static Future<_ProbeResult> _performRequest(Uri uri) async {
    ...
    try {
      ...
      await response.drain().timeout(_timeout);
