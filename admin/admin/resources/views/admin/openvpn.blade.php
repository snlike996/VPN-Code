...
<div class="container-fluid p-0 min-vh-100 bg-light">
    ...
    <div class="row mb-4 px-3 pt-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h4 class="text-dark fw-bold">所有 OpenVPN 服务器</h4>
...
<div class="modal fade" id="modalAddServer" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content bg-light text-dark">
            ...
            <form action="{{ route('admin.openvpn.add') }}" method="POST" enctype="multipart/form-data">
                @csrf
        ...
        <div class="modal-content bg-light text-dark">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label>类型</label>
                        <select id="type" id="type" name="type" class="form-control">
                            <option value="0">免费</option>
                            <option value="1">会员</option>
                        </select>
                    </div>
...
function openEditModal(id, name, country_code, city_name, link, username, password, type) {
    ...
    $('#editForm').attr('action', '{{ route("admin.openvpn.update", ":id") }}'.replace(':id', id));
    $('#name').val(name);
...
function confirmDelete(serverId) {
    if(confirm("确定要删除此服务器吗?")) {
        window.location.href = "{{ route('admin.openvpn.delete', ':id') }}".replace(':id', serverId);
    } else {
...
$(document).ready(function () {
    $('.status-toggle-n').on('change', function () {
        ...
        $.ajax({
            url: '{{ route("admin.openvpn.status", ":id") }}'.replace(':id', serverId),
            type: 'POST',
