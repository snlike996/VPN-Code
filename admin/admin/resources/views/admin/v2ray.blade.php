...
<div class="container-fluid p-0 min-vh-100 bg-light">
    ...
    <div class="row mb-4 px-3 pt-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h4 class="text-dark fw-bold">所有 V2Ray 服务器</h4>
           ...
           <div class="table-responsive shadow-sm rounded">
            <table class="table table-striped table-light text-dark mb-0 align-middle">
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="text-center text-danger">未找到服务器</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="modalAddServer" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content bg-light text-dark">
            ...
            <form action="{{ route('admin.v2ray.add') }}" method="POST">
                @csrf
...
@push('script')
<script>
function openEditModal(id, name, country_code, city_name, link, type) {
    $('#editModal').modal('show');
    $('#editForm').attr('action', '{{ route("admin.v2ray.update", ":id") }}'.replace(':id', id));
    $('#name').val(name);
...
function confirmDelete(serverId) {
    if(confirm("确定要删除此服务器吗?")) {
        window.location.href = "{{ route('admin.v2ray.delete', ':id') }}".replace(':id', serverId);
    } else {
...
$(document).ready(function () {
    $('.status-toggle-n').on('change', function () {
        ...
        $.ajax({
            url: '{{ route("admin.v2ray.status", ":id") }}'.replace(':id', serverId),
            type: 'POST',
